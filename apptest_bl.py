...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///db/air_quality_db_bl_test8.sqlite"
db = SQLAlchemy(app)
...
def index():
    """Return the homepage."""
    return render_template("index_bl.html")
...
@app.route("/ozone_data/<state_county>/<year>")
def get_statecounty(state_county, year, ozone_data = ozone_data):
    """Return a list of sample names."""
    """Return the MetaData for a given sample."""
    sel = [
        ozone_data.arithmetic_mean,
    ...
    ozone_data = {}
    for result in results:
        ozone_data["arithmetic_mean"] = result[0]
        ozone_data['state_county'] = result[1]
        ozone_data['year'] = result[2]
...
def all_aqi(aqi_data = aqi_data):
    ...
    for result in results:
        newEntry = {
            ...
            "fips": result[13],
            "good_percentage": result[14],
            "moderate_percentage": result[15],
            "unhealthy_sensitive_percentage": result[16],
            "unhealthy_percentage": result[17],
            "very_unhealthy_percentage": result[18],
            "hazardous_percentage": result[19],
            "id":result[20]
        }
